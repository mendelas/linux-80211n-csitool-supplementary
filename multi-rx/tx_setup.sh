#!/bin/bash
# TX 電波設定+レート設定のみ（注入はしない）。制御PCから ssh で叩かれる。
# usage: tx_setup.sh [CH] [BW] [RATE]
CH=${1:-165}; BW=${2:-HT20}; RATE=${3:-0x4101}; IF=${4:-wlan1}
...
echo -n "TX freq: "; iwconfig mon0 | grep -o '[0-9.]* GHz'
